...
Prints the signed entry as base64.
`
...
func runSign(args []string, stdout, stderr io.Writer, getenv func(string) string) error {
	...
	if *secretEnv == "" {
		return fmt.Errorf("--secret-env is required: name the environment variable holding the seed")
	}

	seed := getenv(*secretEnv)
	if seed == "" {
		return fmt.Errorf("environment variable %s is empty or unset", *secretEnv)
	}
	...
	if err != nil {
		return fmt.Errorf("the value of %s is not a valid Stellar key", *secretEnv)
	}
	full, ok := parsed.(*keypair.Full)
	if !ok {
		return fmt.Errorf("the value of %s is a public key; a secret seed (S…) is required", *secretEnv)
	}
